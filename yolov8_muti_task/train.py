...
if __name__ == "__main__":
    ...
    #------------------------------------------------------#
    train_annotation_path   = ['split/{0}_2007_train.txt'.format(chr(ord('a')+i)) for i in range(Task)]
    val_annotation_path     = ['split/{0}_2007_val.txt'.format(chr(ord('a')+i)) for i in range(Task)]
    ...
    if model_path != '':
        ...
        pretrained_dict_all = torch.load(model_path, map_location = device)
        if 'model_dict' in pretrained_dict_all.keys():
            pretrained_dict = pretrained_dict_all['model_dict']
            optimizer_dict = pretrained_dict_all['optimizer_dict']
        else:
            pretrained_dict = pretrained_dict_all
            optimizer_dict = None
    ...
    for val_annotation_path_temp in val_annotation_path:
        with open(val_annotation_path_temp, encoding='utf-8') as f:
            val_lines_temp = f.readlines()
            val_lines.extend(val_lines_temp)
    ...
    if True:
        ...
        if optimizer_dict != None:
            try:
                optimizer.load_state_dict(optimizer_dict)
                print('优化器参数导入成功')
            except:
                print("optimizer_dict load failed")
