...
def decode_box(inputs):
    ...
    dbox, cls, _,_,_, anchors, strides = inputs
...
if __name__ == '__main__':
    ...
    # input_data = np.array(image.resize((640, 640))).astype(np.float32)
    # input_data = np.expand_dims(image_data, axis=0).transpose(0, 3, 1, 2)
    ...
    t1 = time.time()
    for _ in range(100):
        outputs_ = sess.run(None, {input_name: image_data})
        for i in range(len(outputs_)):
            np.save('./tensor_cp/onnx/{0}.npy'.format(i), outputs_[i])
            outputs_[i] = torch.from_numpy(outputs_[i])
            # print(outputs_[i].shape)
        # outputs = [decode_box(i) for i in outputs]

        outputs = []
        i = 0
        while (i < len(outputs_) // 7):
            outputs.append(decode_box(outputs_[i * 7:(i + 1) * 7]))
            i += 1
